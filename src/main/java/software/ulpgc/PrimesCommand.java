...
public class PrimesCommand implements Command {
    ...
    private String primesBetween(String lowerBound, String upperBound) {
        return format(
                IntStream.rangeClosed(parseInt(lowerBound), parseInt(upperBound)).filter(this::isPrime).boxed().collect(toList())
        );
    }
    ...
    private String format(List<Integer> primes) {
        return primes.stream().map(String::valueOf).
                collect(Collectors.
                joining(", "));
    }
}
